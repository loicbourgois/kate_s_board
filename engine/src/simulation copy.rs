...
impl Simulation {
    ...
    pub fn sub_tick(&mut self) {
        ...
        unsafe {
            ...
            for l in &mut self.links {
                ...
                let dd = dist - l.length;
                l.stress = dd / l.length;
                if l.stress > l.stress_limit {
                    links_to_delete.push(l.idx);
                    continue;
                }
                let u1 = dd * d.x * l.strength;
                let u2 = dd * d.y * l.strength;
                let u3 = dd * d.x * l.strength;
                let u4 = dd * d.y * l.strength;
                n1.dv.x += u1;
                n1.dv.y += u2;
                n2.dv.x -= u3;
                n2.dv.y -= u4;
    ...
    // fn delete_link_3(&mut self, l: &mut Link) {
    //     // assert!(l.uid == uid);
    //     // assert!(l.active);
    //     l.active = 0;
    //     self.linked.get_mut(&l.a).unwrap().remove(&l.b);
    //     self.linked.get_mut(&l.b).unwrap().remove(&l.a);
    //     self.links_inactive.insert(l.idx);
    // }
